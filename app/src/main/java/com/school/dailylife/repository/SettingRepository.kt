...
class SettingRepository: BaseRepository<SettingService>() {
    ...
    fun uploadContactWayPhoto(part: MultipartBody.Part): Observable<JsonWrapper<Any>> {
        return observable(service.uploadContactWay(CurrentUser.getCurrentUser().token, part))
    }
